...
PATTERNS_VERSION = "v1.0.1"

# Dates: 31/12/2024, 2024-12-31, 31-12-2024
DATE_RE = re.compile(
    r"(?:^|[^0-9])((?:\d{1,2}[./-]){2}\d{2,4}|\d{4}-\d{2}-\d{2})(?:$|[^0-9])"
)

# Invoice number: "Facture n° 123-ABC", "Invoice # 2024-001"
# NOTE: tiret placé À LA FIN de la classe pour éviter l'interprétation "range"
FACTURE_NO_RE = re.compile(
    r"(?:facture\s*(?:n°|no|num(?:éro)?|#)?\s*[:\-\s]*|invoice\s*(?:no|#)?\s*[:\-\s]*)([A-Za-z0-9._/-]+)",
    re.IGNORECASE,
)
INVOICE_NUM_RE = re.compile(
    r"(?:\b(?:n°|no|num(?:éro)?|#)\s*[:\-\s]*)([A-Za-z0-9._/-]+)",
    re.IGNORECASE,
)

# Money like 1 234,56 or 1234.56 or 1.234,56
EUR_STRICT_RE = re.compile(r"\b\d{1,3}(?:[\s\u00A0.]\d{3})*(?:[.,]\d{2})\b")

# Totals proximity labels
...
# VAT rate like 20%, 5.5 %
VAT_RATE_RE = re.compile(r"(\d{1,2}(?:[.,]\d)?\s*%)")

# Parties blocks heuristics
