...
# Lignes article (fallback texte) : "PREST-001 — Libellé ...  12  10,00 €  120,00 €"
LINE_RX = re.compile(
...
# OCR helpers
# =========================
def _has_text_layer(path: Path, min_len: int = 40) -> bool:
    try:
        t = _pm_extract_text(path) or ""
        return len(t.strip()) >= min_len
    except Exception:
        return False

def _preprocess_pil_for_ocr(img: Image.Image) -> Image.Image:
    """Grayscale + léger threshold / deskew (si OpenCV dispo)."""
    if cv2 is None:
        return img.convert("L")
    # PIL -> OpenCV
    cv_img = cv2.cvtColor(np.array(img), cv2.COLOR_RGB2BGR) if 'np' in globals() else None
    if cv_img is None:
        return img.convert("L")
    gray = cv2.cvtColor(cv_img, cv2.COLOR_BGR2GRAY)
    # binarisation adaptative légère
    proc = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_MEAN_C,
                                 cv2.THRESH_BINARY, 21, 10)
    # (deskew léger possible ici)
    return Image.fromarray(proc)

def _ocr_pdf_to_text(path: Path, lang: str = "fra+eng", dpi: int = 300, max_pages: int = 5) -> str:
    if pytesseract is None or convert_from_path is None:
        return ""
    pages = convert_from_path(str(path), dpi=dpi)  # poppler-utils requis
    text_parts: List[str] = []
    count = 0
    for pil in pages:
        count += 1
        ...
        # prétraitement léger
        try:
            pil2 = _preprocess_pil_for_ocr(pil)
        except Exception:
            pil2 = pil.convert("L")
        # OCR
        try:
            txt = pytesseract.image_to_string(pil2, lang=lang, config="--oem 1 --psm 6")
        except Exception:
            txt = ""
        if txt:
...
def extract_pdf(path: str, ocr: str = "auto") -> Dict[str, Any]:
    ...
    used_ocr = False
    if ocr == "never":
        text = _pm_extract_text(p) or ""
    elif ocr == "always":
        text = _ocr_pdf_to_text(p) or (_pm_extract_text(p) or "")
        used_ocr = True
    else:  # auto
        if _has_text_layer(p):
            text = _pm_extract_text(p) or ""
        else:
            text = _ocr_pdf_to_text(p) or ""
            used_ocr = True
    ...
    # 4) Lignes (plumber -> fallback regex)
